            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    using (var scope = _serviceProvider.CreateScope())
                    {
                        ...
                        // process
                        var auctionsToClose = await scope.ServiceProvider.GetRequiredService<IDBRepository>().GetAsync<Post>(
                            s => s.PostType == PostType.Auction &&
                                 s.AuctionStatus == AuctionStatus.Opening &&
                                 s.EndTime <= nextMinute);
                ...
                // Add a short delay to prevent immediate loop iteration
